...
- [x] 1.1 Apply the ground, accent, and text-ramp values from `design.md — Decision 1` to the
...
- [x] 1.2 Add `--color-scrim`, derived from `--color-void`, to the same block; verify it appears in
...
- [x] 1.3 Replace the hardcoded `rgba(4, 4, 10, 0.95)` washes in `src/components/card/card.css`
...
- [x] 1.4 Verify contrast: check `text-dim`, `text-muted`, and `text-bright` against the new
...
- [x] 1.5 Walk `/battle`, `/shop`, `/cards`, `/game`, and `/style-guide` on the new palette and
...
- [x] 2.1 Change `--dur-entrance` to `0.5s` and add `--dur-stagger: 0.09s` to the `:root` motion
  block in `src/styles/tokens.css`; verify both appear under the style guide's Motion section.
- [x] 2.2 Rewrite `.fade-in` in `src/components/battle/battle.css` to derive its delay from
...
- [x] 2.3 Replace the seven inline `--delay` literals in `src/components/battle/battle.html` with
...
- [x] 2.4 Verify the entrance settles in about a second: load `/battle` and confirm the Start Game
...
- [x] 3.1 Hoist the resting soul glow into a `--glow-rest` custom property on `.title-hollow` and
  apply it as a static `text-shadow`; verify the title renders lit with animations disabled in
  devtools.
- [x] 3.2 Rewrite `soulFlicker` so its `0%`/`100%` stops read `text-shadow: var(--glow-rest)`, and
...
- [x] 3.3 Give `.title-master` the same treatment for gold — static resting `text-shadow`, and
...
- [x] 3.4 Verify no pop: load `/battle` and watch through the entrance and at least one full flicker
...
- [x] 3.5 Update the reduced-motion block so the static resting glow is what remains when animations
  are off; verify with `prefers-reduced-motion: reduce` emulated that the title is visible, lit, and
  still.

## 4. Particle field generator

- [x] 4.1 Create `src/model/particle.ts` with the `Particle` interface, a `mulberry32` PRNG, and
...
- [x] 4.2 Add `particleVars(p)` to the same module, returning the CSS custom-property map the
  templates bind; verify every property the stylesheet reads has a corresponding entry with units.
- [x] 4.3 Write `src/model/particle.spec.ts` covering: same seed yields identical fields; different
...
- [x] 5.1 Rewrite `.particle` in `src/styles/primitives.css` as the travelling box — position from
...
- [x] 5.2 Add the `.particle::after` dot carrying background, glow, `filter: blur(var(--blur))`,
...
- [x] 5.3 Add the `particleRise`, `particleFall`, `particleSway`, and `particleTwinkle` keyframes and
  the `.particle.falls` variant; verify a falling particle descends from above the surface and that
  only `transform` and `opacity` animate.
- [x] 5.4 Update the reduced-motion block to rest particles at their generated `--x`/`--y` with
...
- [x] 6.1 Convert `src/components/battle/` to the generator: replace the index array with
...
- [x] 6.2 Convert `src/components/game/` the same way at its existing count of 14; verify the
  in-game screen still satisfies the `CLAUDE.md` fit constraints.
- [x] 6.3 Convert `src/components/shop/` (14) and `src/components/collection/` (14); verify both
  routes render a populated field immediately on navigation.
- [x] 6.4 Convert `src/components/open-pack/` (18); verify its field renders and no
  `--particle-gap` declaration remains in its CSS.
- [x] 6.5 Convert the style guide's atmosphere demo — replace the inline `[0..11]` array in
...
- [x] 7.1 Run `ng build` and `ng test`; verify both succeed with no new warnings.
- [x] 7.2 Run Prettier over the changed files and verify formatting is clean at the project's
  100-char / single-quote / angular-parser settings.
- [x] 7.3 Review `/style-guide` end to end: confirm the token tables show the lifted palette and the
...
- [x] 7.4 Walk the full navigation loop — Battle → Shop → Cards → Battle → Start Game — and confirm
