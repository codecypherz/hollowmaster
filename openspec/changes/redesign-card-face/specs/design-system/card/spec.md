...
**Reason**: The face gains a fourth section. Ability text is now part of what a card presents, so a contract fixed at exactly three sections can no longer describe it. Restated below as "Four-section card face", with every guarantee the three-section requirement made (ordering, delineation, long-name truncation, missing-artwork resilience) carried forward unchanged.

**Migration**: Surfaces need no change — they already delegate to the shared renderer and pass no layout information. Anything asserting that the face has exactly three children must be restated against four. The three original sections keep their order and their relative positions; the Ability section is appended below Stats.

### Requirement: The card face presents only name, artwork, stars, and stats

**Reason**: Superseded by an explicit product decision. The card model has carried `ability`, `set`, and `number` since the data-model change, and the reference design places all three on the face. This requirement's prohibition is inverted rather than relaxed: those properties are now required on the face, not merely permitted.

**Migration**: Nothing outside the renderer read this requirement. Any surface that was showing ability, set, or number *because* the face would not should be reviewed for duplication once the face shows them. `CLAUDE.md` carries a stale duplicate of this constraint; it is removed there in the same change, leaving this spec as the single authority.

### Requirement: Star rating rendered as stars

**Reason**: The star display no longer draws one star per level and nothing else. It becomes a fixed five-slot track that shows the ceiling on every card, plus a sixth star rendered outside the track and only at a rating of 6. "One star per level" and "N stars appear" no longer describe what is drawn. Restated below as "Star rating rendered on a five-slot track".

**Migration**: The card model is unchanged — the rating is still an integer from 1 to 6 and the renderer still applies no bound of its own, since the model guarantees the range. Every unchanged guarantee (top-left of the image section, never a numeral, legible over artwork) is carried forward. Anything asserting that the rendered star count equals the rating must be restated: for ratings 1-5 the count of *lit* stars equals the rating and five slots are always present.
